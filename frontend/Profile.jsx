  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setIsOwnProfile(currentUserId === userIdToFetch || currentUserId === userData.id);
                ...
                <>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <input
                      ...
                      placeholder="Year"
                    />
                    <label className="flex items-center gap-2 px-3 py-1 bg-gray-50 border border-gray-200 rounded-lg text-sm cursor-pointer hover:bg-gray-100">
                      <Upload className="w-4 h-4 text-gray-500" />
                      <span className="text-gray-600">File</span>
                      <input
                        type="file"
                        className="hidden"
                        onChange={(e) => setNewQualification({...newQualification, file: e.target.files[0]})}
                      />
                    </label>
                    <button
                  ...
                  {newQualification.file && (
                    <p className="text-xs text-gray-500">Selected file: {newQualification.file.name}</p>
                  )}
                </>
              )}
              ...
              {isOwnProfile && (
                <div className="flex flex-wrap gap-2">
                  <select
                    ...
                    className="px-3 py-1 border border-gray-200 rounded-lg text-sm focus:border-[#00B330] outline-none bg-white"
                  >
                  ...
                  <input
                    ...
                    className="w-24 px-3 py-1 border border-gray-200 rounded-lg text-sm focus:border-[#00B330] outline-none"
                    placeholder="Start"
                    type="time"
                  />
                  <input
              ...
              {user.availability?.length > 0 ? (
                <div className="space-y-3">
                  {user.availability.map((avail) => (
                    <div key={avail.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                      <div className="w-10 h-10 rounded-full bg-purple-50 flex items-center justify-center flex-shrink-0">
                        <Clock className="w-4 h-4 text-purple-600" />
                      </div>
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-900">{avail.day}</h4>
                        <p className="text-sm text-gray-600">{avail.start} - {avail.end}</p>
                      </div>
                      {isOwnProfile && (
                        <button
